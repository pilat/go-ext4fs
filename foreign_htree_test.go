...
// image carrying an unmaintainable ro_compat feature (uninit_bg/gdt_csum), reserved
// GDT blocks (resize_inode), or no sparse_super is refused rather than silently
// mis-read and corrupted on the next Save. Each image otherwise passes the geometry
// and incompat checks, so they exercise these guards specifically. (metadata_csum is
// NOT here — its checksums are maintained on write, so such images are accepted; see
// the WithChecksum reopen tests.)
